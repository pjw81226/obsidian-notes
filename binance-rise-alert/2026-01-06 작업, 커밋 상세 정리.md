...
[[Kafka 토픽 컷오버(Cutover)로 심볼 키 기반 파티션 확장 문제 해결]]
### 작업 목표 요약
...
### 왜 했나
- Discord Webhook URL이 코드에 들어가 있으면 레포에 푸시되는 순간 시크릿 유출
- 실행 시점에만 주입되도록 바꿔서 유출면적을 줄임
...
### 왜 했나
- Binance WebSocket은 네트워크/서버 사유로 언제든 끊길 수 있는데 끊김 시 즉시 종료하면 데이터 수집이 멈춘다.
...
### 왜 했나
- Kafka 전송은 순간 장애/재시도로 인해 중복/유실 가능성이 생길 수 있어 idempotence 기반으로 내구성을 올림
...
### 왜 했나
- “심볼을 key로 파티션에 태우는 구조”에서 **파티션 수를 늘리면** key→partition 매핑이 바뀌어 심볼 순서 보장이 깨질 수 있다.
- 그래서 파티션 확장은 “기존 토픽 증설” 대신 **새 토픽(v2) 생성 → Producer/Flink 컷오버**가 가능한 구조로 바꿈
...
### 왜 했나
- Flink에서 이벤트마다 `System.out.println`을 찍으면 트래픽 증가 시 병목/비용/로그 폭증
...
### 왜 했나
- 기존 `VolumeSpike`는 aggTrade를 단순 카운트했다가 kline final에서 리셋하는 구조라, **aggTrade가 어떤 kline 구간에 속하는지**가 애매해져 정합성이 흔들릴 수 있음
...
### 왜 했나
- Discord는 레이트리밋(429)이 자주 나고 일시 장애(5xx)도 있을 수 있는데 현재는 실패 시 그냥 로그만 남겨 알림이 끊길 수 있음

### 변경 내용
- Discord 전송을 즉시 HTTP로 보내지 않고, 내부 큐(최대 1000) + 단일 워커 스레드**로 직렬 처리
...
### 주의/추가 개선 후보
- 현재 Kafka consumer는 auto commit임. “Discord 전송 실패”가 나도 Kafka 오프셋은 진행된다.
  알림 at-least-once가 필요하면 **수동 커밋/재처리 전략**을 별도 설계하는 게 좋을듯.
...
- [ ] v2 토픽 전략에 맞춰 토픽명/파티션 env 세팅 후 컷오버 계획 수립
- [ ] auto commit을 수동으로 변경하고 at least once가 제대로 동작하도록 변경하는게 좋을듯